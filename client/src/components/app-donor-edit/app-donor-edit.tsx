...
export class AppDonorEdit {
  ...
  async componentWillLoad() {
    try {
      const donor = await donorService.findOne(this.donorId);
      this.title = donor.title ?? '';
      this.firstName = donor.firstName;
      this.lastName = donor.lastName;
      this.organizationName = donor.organizationName ?? '';
      this.address1 = donor.address1 ?? '';
      this.address2 = donor.address2 ?? '';
      this.city = donor.city ?? '';
      this.usState = donor.state ?? '';
      this.postalCode = donor.postalCode ?? '';
      this.contacts = donor.contacts.map((c) => ({
        id: c.id,
        type: c.type as ContactRow['type'],
        label: c.label as ContactRow['label'],
        value: c.value,
      }));
    } catch (err: any) {
      this.error = err.message ?? 'Failed to load donor';
    } finally {
      this.loading = false;
    }
  }
      ...
      <div class="min-h-screen bg-gray-50">
        <div class="max-w-2xl mx-auto px-6 py-8">
          <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
            ...
            <span>/</span>
            <span class="text-gray-900 font-medium">Edit donor</span>
          </div>

          <h2 class="text-2xl font-bold text-gray-900 mb-8">Edit donor</h2>
